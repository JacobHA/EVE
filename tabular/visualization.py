...
def plot_dist(desc, *paths_list, ncols=4, filename=None, titles=None, main_title=None, figsize=None, show_values=False, show_plot=True, symbols_in_color=True, symbol_size=180, dpi=300, normalize=True):
    ...
    if len(paths_list) == 0:
        ...
        print('Single path provided, creating a single plot...')
        # fig = plt.figure(figsize=figsize)
        # axes = [plt.gca()]
    elif len(paths_list) > 1:
        print('Multiple paths provided, creating subplots...')
    ...
    for axi, paths, title in zip(axes, paths_list, titles):
        ...
        if type(paths) == dict:
            ...
            draw_paths(desc, axi, paths, title, show_values,
                       symbols_in_color, symbol_size, normalize=normalize)
...
def draw_paths(desc, axi, paths, title=None, show_values=False, symbols_in_color=True, symbol_size=120, normalize=True):
    if paths is None:
        return
